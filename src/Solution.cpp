...
bool clsSolut::Grouping()
{
	...
	while(nextR !=  this->mFile->end())
	{
		...
		typedef pair<size_seq_tot, id_seq_type> Weight_Recurrence__Border; //Salva il peso dell'arco e quante volte viene visto analizzando la frontiera
...
void clsSolut::MergeGroup()
{
	...
	if(this->param->estimateK)
	{
		...
		cout << "... Complete. K is: " << gmeans.getEstimateK() << flush;
