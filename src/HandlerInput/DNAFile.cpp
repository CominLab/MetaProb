...
clsDNAFile::clsDNAFile(vector<PairFiles> filename, Spaced_Qmer Q, TypeGraph graph, size_seq m_thres):
filename(filename), Q(Q), graph(graph), m_thres(m_thres)
{
	this->seq_tot = 0;
	this->qMer_Max = 0;
}
...
bool clsDNAFile::ExtractQMerFromFile()
{
	...
	this->mHash->InitOptionData(this->Q, this->qMer_Max*0.8);
...
inline bool clsDNAFile::ScanSingleEndFileToCallFunction(PairFiles& fileScan, int (clsDNAFile::*functocall)(PairFiles&, id_seq_type&, string&, string&))
{
	...
	if(file.is_open())
	{
		...
		while(getline(file,line))
		{
			if(!line.empty())
			{
				if(line[0] == fileScan.first.getSequenceDelimiter())
				{
					//due casi:
...
inline bool clsDNAFile::ScanPairedEndFileToCallFunction(PairFiles& fileScan, int (clsDNAFile::*functocall)(PairFiles&, id_seq_type&, string&, string&))
{
	...
	if(file1.is_open() && file2.is_open())
	{
		...
		while(getline(file1,line1) && getline(file2,line2))
		{
			...
			{
				if(line1[0] == fileScan.first.getSequenceDelimiter() && line2[0] == fileScan.second.getSequenceDelimiter())
				{
					//due casi:
					//o delimitatore non fa parte di qualità se isLineQuality == false, quindi line contiene la descrizione del read
					//o delimitatore fa parte di qualità se isLineQuality == true e line contiene la riga qualità
...
inline int clsDNAFile::storeSequencesInfo(PairFiles& fileScan, id_seq_type& idxFile, string& seq1, string& seq2)
{
	...
	++this->seq_tot;
	this->qMer_Max += seq1.length() - this->Q.GetQ() + 1;
	fileScan.first.AddSequenceMap(idxFile, mySeqPE1->getSeqId()); //Mappa idxFile in idxApp
	...
	if(fileScan.getType() == PairedEnd)
	{
		if(this->graph == Single || this->graph == SingleUnion)//Salva singoli
		{
			...
			++this->seq_tot;
			this->qMer_Max += seq2.length() - this->Q.GetQ() + 1;
		...
		fileScan.second.AddSequenceMap(idxFile, mySeqPE2->getSeqId());//Mappa idxFile in idxApp
	}
...
//{
//	++idxFile;
//	if(seq1.size() >= this->Q.GetQ())
//		this->repeatQmer->insertForRepeatSearch(seq1);
//	if(seq2.size() >= this->Q.GetQ())
...
//		GetHashes_speedup_previous(str, this->Q, vHash, CharToInt);
//		GetHashes_speedup_unit(str, this->Q, vHash, CharToInt);
		GetHashes_naive(str, this->Q, vHash, CharToInt);
		...
		LMerVectorCompress::Ptr compr_vLmer)
{
	...
	if(seq.length() >= l_mer_size)
	{
		...
		GetHashes_speedup_previous(seq, l_mer_size, hashes, CharToInt);
