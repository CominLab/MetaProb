...
int main(int argc, char* argv[])
{
	...
	//Default parameter
	param->nClus = 0;
	param->q = Spaced_Qmer("111111111111111111111111111111");
	param->m_thres = 5;
	param->SEEDSIZE = 9000;
	param->l_mer_freq = 4;
	param->iteration = 100;
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i], "-q") == 0)
		{
			...
			if(spaced_qmer.front() != '1' || spaced_qmer.back() != '1')
			{
				...
				param->q = Spaced_Qmer(spaced_qmer);
		}
		...
		else if(strcmp(argv[i], "-feature") == 0)
		{
			...
			{
				cerr<<endl<<"-feature != [1, ..., 12]. Applied default parameter 9. "<< Normalization::enum_string(Normalization::NORM_D2star_All_Read_Prob_Lmer_Euclidian) <<flush;
				param->norm_type = Normalization::NORM_D2star_All_Read_Prob_Lmer_Euclidian;
			}
			else
